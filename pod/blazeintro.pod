...
data (e.g. configuration, blog posts and static pages) intact. Optionally,
you can force the utility to restore the configuration, theme, style and
language files to their initial state by adding the following option:
...
Similarly to blog posts, you can manage the static pages using the same
commands with additional C<--page> (or C<-p> for short) option:
