...
style and language files to their initial state like this:
...
body for you to write---do not forget to use HTML! You can also use a
special placeholder, C<%root%>, as a relative path to the base URL
(directory where the blog is placed) in your links and images. For example,
to link the RSS feed from your blog post, type something like this:
...
To browse static pages instead of blog posts, simply add the C<--pages>
option as follows:
...
paths creation to make it easier to browse the off-line content like this:

  $ blaze-make --full-paths

Furthermore, if the public web directory is located on the same server, you
can create the content directly in that location like this:
